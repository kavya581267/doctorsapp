...
export const patientService = {
    ...
    moveNoteToInprogress: async (clinicId: number, patientId: number, noteId: number): Promise<any> => {
        const body = {
            clinicId: clinicId,
            patientId: patientId,
            noteId: noteId
        ...
        try {
            const resp = await apiService.put(MOVE_TO_IN_PROGRESS, body);
            return resp.data;
        } catch (error) {
            throw error
        }
